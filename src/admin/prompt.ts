...
export const getChoice = async (message: string, choices: string[]): Promise<string> => {
  const result = await inquirer.prompt({
    message: `${message}?`,
    name: "answer",
    type: "list",
...
const untilCorrect = async (promptFunc: () => any, isCorrectMessage?: string) => {
  while (true) {
    const answer = await promptFunc();
    const shouldExit = await confirm(isCorrectMessage ? isCorrectMessage : `Is "${answer}" correct`);
    if (shouldExit) {
      return answer;
    }
  }
...
export const getStringUntilCorrect = async (message: string, isCorrectMessage?: string) => {
